...
class Redisdb {
	...
	public function __construct($group = array()) {
		...
		$config = $CI->config->item('redis')['redism'];
		$this->_config = $config[$group['key']];
		$this->_group = $group['key'];
		...
		switch ($method) {
			case 2:
				if (true === empty($this->_conn[$this->_group]['master'])) {
					...
					$this->_conn[$this->_group]['master']->auth($host[3]);
					$this->_conn[$this->_group]['master']->select($host[5]);
				}
				break;
			case 1:
			default:
				if (true === empty($this->_conn[$this->_group]['slave'])) {
					...
					$this->_servers['host'] = $host[0];
					$this->_servers['port'] = $host[1];
					$this->_conn[$this->_group]['slave']->auth($host[3]);
					$this->_conn[$this->_group]['slave']->select($host[5]);
				}
				break;
		}
	}
